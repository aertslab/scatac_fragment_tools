...
def run_split_command(tmp_path, output_folder, file_dict):
    ...
    path_to_a_fragment_index = os.path.join(
        TEST_DIRECTORY, file_dict["a.fragment_index"]
    )
    path_to_b_fragments = os.path.join(TEST_DIRECTORY, file_dict["b.fragments"])
    path_to_b_fragment_index = os.path.join(
        TEST_DIRECTORY, file_dict["b.fragment_index"]
    )
    path_to_sample_to_fragment = os.path.join(
        TEST_DIRECTORY, file_dict["sample_to_fragment"]
    )
    path_to_cell_type_annotation = os.path.join(
        TEST_DIRECTORY, file_dict["cell_type_annotation"]
    )
...
def split_command_test_helper(tmp_path, file_dict):
    ...
    cell_annotations = pl.read_csv(
        TEST_DIRECTORY.joinpath(file_dict["cell_type_annotation"]), separator="\t"
    )

    for row in cell_annotations.select(pl.col("cell_type")).unique().iter_rows():
        cell_type = row[0]
        cell_type_bcs = (
            cell_annotations.filter(pl.col("cell_type") == cell_type)
        ...
        assert_frame_equal(fragments_cell_type, generated_fragments_cell_type)
...
def split_command_test_helper_w_sample_id(tmp_path, file_dict):
    ...
    for (cell_type,) in (
        cell_annotations.select([pl.col("cell_type")]).unique().iter_rows()
    ):
        cell_type_bcs = (
            cell_annotations.filter(pl.col("cell_type") == cell_type)
            .get_column("cell_barcode")
        ...
        fragments_cell_type = pl.concat(
            [
                ...
                ).with_columns((pl.lit("A_") + pl.col("column_4")).alias("column_4")),
                b_fragments.filter(
                    pl.col("column_4").is_in(cell_type_bcs)
                ).with_columns((pl.lit("B_") + pl.col("column_4")).alias("column_4")),
            ]
        ).sort(by=["column_1", "column_2", "column_3", "column_4"])
        generated_fragments_cell_type = pl.read_csv(
            os.path.join(output_folder, f"{cell_type}.fragments.tsv.gz"),
...
def test_split_command_w_sample_id_bc_single_type(tmp_path):
    split_command_test_helper_w_sample_id(
        tmp_path, FILES_ALL_BARCODES_MAPPING_TO_SINGLE_TYPE
    )


def test_split_command_w_sample_id_barcode_mapping_multiple_types(tmp_path):
    split_command_test_helper_w_sample_id(
        tmp_path, FILES_SOME_BARCODES_MAPPING_TO_MULTIPLE_TYPES
    )
